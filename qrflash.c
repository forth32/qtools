...
bzero(partlist,sizeof(partlist)); // очищаем список разрешенных к чтению разделов
...
-f n      - читать только раздел под номером n, ключ может быть указан несколько раз для формирования списка разделов\n\
